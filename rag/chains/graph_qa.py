...
def trim_query(query: str) -> str:
    """
    Trims a SPARQL query to remove unnecessary parts and returns only the essential query (WHERE) components.
...
class GraphSparqlQAChain(Chain):
    ...
    def _call(
        ...
        if intent == "SELECT":
            # graph KB context first
            graph_context = self.graph.query(generated_sparql)

            _run_manager.on_text("Full Context:", end="\n", verbose=self.verbose)
            _run_manager.on_text(
                str(graph_context), color="green", end="\n", verbose=self.verbose
            )
            ...
            result = self.qa_chain(
                {"prompt": prompt, "context": combined_context, "query": generated_sparql},
                callbacks=callbacks,
            )
